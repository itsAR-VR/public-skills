...
description: Use the Supermemory CLI to programmatically manage memories, documents, profiles, tags, connectors, keys, and team/account workflows from the terminal. Use when scripting Supermemory setup, add/search/remember/forget/update/profile flows, scoped keys, connector syncs, or machine-readable CLI output.
---

The Supermemory CLI is the terminal interface for Supermemory. It lets you add memories, search, manage documents, configure projects, connect external data sources, and administer teams — all programmatically.

Source check (2026-06-11): Supermemory's official changelog describes the CLI as a command-line tool for managing memories, documents, profiles, tags, connectors, and API keys from the terminal, and the official site shows `npx supermemory setup` as the setup entrypoint. Before relying on rarely used flags, run `supermemory help --all` or the specific command's `--help` because CLI surfaces can drift.

## Installation & Auth

```bash
# Setup via the official npm entrypoint
npx supermemory setup

# Authenticate (opens browser OAuth flow)
supermemory login

# Or use an API key directly if your installed CLI supports direct login
...
# Create a scoped key (restricted to one or more container tags)
...
When using a scoped API key (restricted to one or more container tags), only these commands are available:
