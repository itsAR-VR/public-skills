...
### Limitation
The Atlassian MCP does not currently support creating issue links. If the CLI
backend is available, use `jira issue link` from `references/commands.md`.
If neither backend can create the link, stop and tell the user link creation
needs a configured Jira CLI or Jira API path.
