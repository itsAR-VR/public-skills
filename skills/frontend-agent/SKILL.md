...
These rules are embedded from `ecc-frontend-design` (primary design doctrine) and OpenAI frontend design guidance captured at the 2026-06 refresh. Re-check current official OpenAI frontend/model guidance before updating model-specific rules. The agent MUST follow all of them.
...
Task → Skills to Load
─────────────────────────────────────────────────────────
Any UI build          → ecc-frontend-design (ALWAYS), ui-skills (ALWAYS)
...
Visual QA             → browse-qa, browser-harness
...
**Default:** Low/Medium for frontend. This is a 2026-06 model-behavior heuristic from the OpenAI guidance captured above: lower reasoning tends to avoid over-architecting and helps the agent commit to design decisions faster. Re-check current official OpenAI guidance before treating this as permanent.
...
| Skill | Path | Role |
|-------|------|------|
| `ecc-frontend-design` | `~/.agents/skills/ecc-frontend-design/SKILL.md` | Primary design doctrine — load first |
...
| `browser-harness` | `~/.agents/skills/browser-harness/SKILL.md` | Automated verification |
...
2. Read ecc-frontend-design SKILL.md (always)
