...
OpenAI's GPT-5.6 preview notice said GPT-5.6 was not available in ChatGPT as of
2026-07-09, so Oracle's browser engine cannot be pinned to this family. Leave
that engine on its current provider-controlled browser default. API engine is
useful when `OPENAI_API_KEY` or Azure config is ready, but its exact deployment
catalog must still advertise the requested ID.

Source: `https://help.openai.com/en/articles/20001325-a-preview-of-gpt-5-6-sol-terra-and-luna`.

Recommended defaults:

- Preview first: `--dry-run summary --files-report`
- Browser long run: `--engine browser` with the current browser default.
- GPT-5.6 exact routing: use the direct Codex CLI tier from the `codex` skill,
  or use Oracle API mode only after `oracle --help` or the provider catalog
  advertises the exact model ID.
