...
description: "FALLBACK ONLY for image/video (default: Higgsfield via `/higgsfield:generate`). Keep using this skill for fal.ai-exclusive audio (CSM-1B TTS, ThinkSound video-to-audio). Unified media generation via fal.ai MCP — image, video, and audio. Covers text-to-image (Nano Banana), text/image-to-video (Seedance, Kling, Veo 3), text-to-speech (CSM-1B), and video-to-audio (ThinkSound). Use when the user wants to generate images, videos, or audio with AI."
origin: ECC
---

> **Routing rule (2026-05-11):** Default image/video generation in this workspace is **Higgsfield AI** via `/higgsfield:generate`. Use **this** skill only for fal.ai-exclusive capabilities (CSM-1B text-to-speech, ThinkSound video-to-audio) or when the user explicitly names fal.ai.
