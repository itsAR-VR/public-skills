...
| **JS-rendered sites** | `browser-harness` (free) | When HTML scraping fails |
...
jobs:
  scrape:
    ...
    steps:
      ...
      # Uncomment if Browser Harness is enabled in requirements.txt
      # - name: Install Browser Harness browsers
      #   run: python -m browser-harness install chromium --with-deps
...
### Pattern 5: JS-Rendered Pages (Browser Harness)
```python
from browser-harness.sync_api import sync_browser-harness

with sync_browser-harness() as p:
...
| JS-rendered sites with `requests` | Empty response | Use Browser Harness or look for the underlying API |
...
# browser-harness==1.40.0   # uncomment for JS-rendered sites
```
