...
```
/graphify                                             # full pipeline on current directory -> HTML/report/JSON
...
Print it once, then continue. If `GEMINI_API_KEY` or `GOOGLE_API_KEY` IS set, use `graphify.llm.extract_corpus_parallel(files, backend="gemini")` for semantic extraction instead of dispatching Claude subagents. Last checked 2026-06-10: the installed `graphify.llm` default Gemini model is `gemini-3-flash-preview`; verify after `graphifyy` upgrades or Gemini model-doc changes. Set `GRAPHIFY_GEMINI_MODEL` or pass `--model` in headless CLI flows to override it.
...
directed = DIRECTED_FLAG  # replace with True if --directed was given, else False
