...
When multiple agents work concurrently on the same codebase, conflicts and
dependencies can arise. This reference provides procedures to detect and handle
these situations.
...
Before starting implementation work, scan the last 10 phases for potential
overlaps:
...
3. **Agent Team / lanes** — Are there active parallel lanes on the same surface?
4. **Phase Summary** (if exists) — What files/artifacts were produced?
...
4. Update your lane boundaries if the shared surface is larger than expected

### Step 3 — Check for Active Concurrent Work

Look for signals that another agent is actively working:

1. **Partially completed plan.md files** — Output Contract / Downstream Consumers sections empty
...
4. **Shared dependency nodes** — multiple plans depend on the same contract, schema, or integration subphase
...
6. **Re-check parallel safety** — If both lanes now need the same file, collapse to serial execution or add an integration owner
...
5. **Prefer synthesis lanes** — If multiple outputs converge, route them through a named integrator instead of direct cross-lane edits
...
- [ ] Verified my lane still has isolated file / contract ownership
- [ ] Identified the synthesis owner for any shared outputs
```
...
**Lane update:** <did this stay parallel-safe, or require serialization?>
```
...
| Parallel lanes collide on same file | Subphase target files drift together | Serialize or create integration owner |
