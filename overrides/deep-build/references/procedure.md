...
ELSE IF active_harness == "claude":
  ...
  "$CODEX_BIN" exec --model "$OPENAI_LATEST_MODEL" -c model_reasoning_effort=ultra --sandbox read-only \
...
Current hardcoded snapshot, locally verified with Codex CLI on 2026-07-09
(see `references/model-currency.md` before changing):

- OpenAI/Codex verifier: `gpt-5.6-sol` with `ultra` effort
- Claude Code highest-capability alias: `fable` when available
- Claude Code current Opus pinned ID: `claude-opus-4-8`
- Claude Code family aliases: `fable`, `opus`, `sonnet`, `haiku`
