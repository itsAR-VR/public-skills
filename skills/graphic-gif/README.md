...
- Captures frames using Browser Harness + Web Animations API for frame-accurate seeking
...
- `scripts/capture-and-encode.mjs` — Browser Harness frame capture + gifenc assembly
- `gifenc` + `sharp` (or `jimp`) + `browser-harness` — auto-installed via npm on first run
