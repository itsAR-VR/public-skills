...
AI-powered SVG icon generation using the configured Gemini text model in `scripts/icon/generate.py`. 15 styles, 12 categories, multi-size export.

Model source check: as of 2026-06-10, this reference describes the local script default only. Before treating Gemini model IDs, names, limits, or availability as current provider truth, verify the Google Gemini API model docs and `scripts/icon/generate.py`.
...
- Local script default as of 2026-06-10: `gemini-3.1-pro-preview`
- Refresh trigger: if the model is unavailable or renamed, verify current Google Gemini API docs before changing this reference or `scripts/icon/generate.py`
