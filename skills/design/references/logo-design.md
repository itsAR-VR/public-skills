...
AI-powered logo design with 55+ styles, 30 color palettes, 25 industry guides. Uses the configured Gemini image models in `scripts/logo/generate.py`.

Model source check: as of 2026-06-10, this reference describes the local script defaults only. Before treating Gemini model IDs, names, image limits, or availability as current provider truth, verify the Google Gemini API image-generation/model docs and `scripts/logo/generate.py`.
