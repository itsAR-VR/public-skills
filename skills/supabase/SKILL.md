...
Do not rely on training data for Supabase features. Function signatures, config.toml settings, and API conventions change between versions. Before implementing, look up the relevant topic using the documentation access methods below. Last source check for the CLI/MCP facts in this skill: 2026-06-11; re-check Supabase docs and CLI releases before relying on version gates or server URLs.
