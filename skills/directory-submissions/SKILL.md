...
**Source freshness:** This skill contains time-sensitive platform and AI-discovery claims. Before giving calendar-specific or platform-policy advice, verify the current source first (official directory docs/status pages where possible), date the check in the output (`source check: YYYY-MM-DD`), and treat unverified stats as directional. Refresh this skill when Product Hunt ranking rules, G2/Capterra ownership or pricing, directory URLs/DR, MCP registry status, or AI-engine citation behavior changes.
...
Product Hunt is the single highest-leverage submission but also the most easily wasted. As of a 2026-06-10 source check, Product Hunt's own launch guidance says not to ask directly for upvotes and to ask people to visit and comment instead; exact ranking weights are volatile, so treat third-party algorithm claims as directional. **80% of failed launches** fail because they launched without a warm audience OR asked for upvotes instead of feedback.
...
- **Day -10:** (Optional) book a hunter only for audience fit or strategic feedback. Product Hunt's current launch guide says makers can hunt their own products and third-party hunters have no discernible advantage.
...
4. **Product Hunt hunter lined up?** (Optional for audience fit or feedback; not required for launch mechanics. 3-week warm-up still matters.)
