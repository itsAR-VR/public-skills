...
> **Source check (2026-06-11):** Treat model-specific limits, pricing, duration,
> resolution, audio support, and API access as volatile. Verify the provider's
> current official docs before relying on any model-specific tip below.
...
- May support audio generation depending on the current model/version
- Best with detailed, descriptive prompts
- Specify the desired resolution only after checking supported output settings
- Can handle multiple subjects and scene transitions

### Runway

- Strong motion control — specify camera movements precisely
- Strong temporal consistency for many image-to-video and motion-control workflows
- Use current Runway controls for specific area animation when available
- Image-to-video works well — provide a reference frame
- Keep prompts under 100 words for best results

### Kling

- Good for longer narrative attempts when current duration limits support them
- Check current pricing and quality tiers before using it for bulk generation
- Expect quality to vary with duration, tier, prompt complexity, and source image quality
- Best with simpler scenes and fewer subjects

### Pika

- Good for quick iterations and experimentation
- Effects modes can add motion to still images when available
- Best for short social-style clips and fast concept testing
...
- **Use lower-cost modes for drafts** — verify current provider pricing before choosing the draft model
