...
description: >-
  ...
  looping video embedded on one editorial webpage. Use when someone wants their day, week,
  launch, or any real timeline rendered as a teaser-trailer sizzle artifact.
metadata:
  source_repo: https://github.com/alexmcdonnell-airtable/hyperagent-public-skills
  source_commit: a1349c1
  source_file: skill-briefing-trailer.json
  hyperagent_name: "briefing-trailer"
  exported_at: "2026-06-01T13:54:26.599Z"
  auth_type: "none"
  tags:
    - "sizzle"
    - "video"
    ...
    - "nano-banana"
  when_to_use: >-
    Use when a user wants a real timeline — their day, a launch week, an on-call shift, a
...
Use the bundled **`scripts/render_text.py`**. Write a `textspec.json` and run:
```
pip install Pillow -q   # if not present
python3 scripts/render_text.py textspec.json
```
...
Spec example is in the docstring of `scripts/render_text.py`. Tune `ink/dim/accent/bg/caption_center_y` to match the world bible. Keep captions to one line; place `caption_center_y` just above the bottom letterbox (~862 for 132px bars).

---

## STEP 5 — Write the cut-sheet and assemble (OOM-safe)

**Why per-segment:** a single ffmpeg filtergraph with ~20+ looped 1080p inputs gets OOM-killed (rc -9 / SIGKILL, then "moov atom not found"). The bundled **`scripts/build_trailer.py`** renders each shot to its own file, concat-demuxes with `-c copy`, then does ONE light grade+audio pass. Peak memory stays tiny and it's far more debuggable.

Write a `cutsheet.json` (schema in the script docstring). Shot types:
- `card` — opaque statement/data PNG, gentle fade in/out
- `footage` — a trimmed window of a Veo clip (`in`/`out`; legacy `start`/`duration` still works), cover-cropped, with optional transparent `caption` PNG overlay
...
```
python3 scripts/build_trailer.py cutsheet.json
```
...
## Bundled scripts
- **`scripts/render_text.py`** — JSON-driven Pillow text renderer (statement/data/caption styles, font auto-detection, true letter-spacing, translucent caption plates). No credentials.
- **`scripts/build_trailer.py`** — JSON-driven, OOM-safe ffmpeg assembler (per-segment render → concat-demux → grade+letterbox+synth audio). No credentials.
