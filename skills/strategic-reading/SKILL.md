...
> **Citation rule:** every recommendation must cite the source (chapter or
> section). The source's own words carry more weight than paraphrase.
...
Where a general personalized read maps a book to the reader's whole life,
...
A structured markdown document is the artifact. PDF is an optional
rendering, never primary.
...
## See Also
Links to related notes or documents (related concepts, related projects).
```
...
Phase 1: Ingest the source
  ├── EPUB: extract chapters (e.g., via BeautifulSoup)
...
Phase 5: Write and deliver
  ├── Write the playbook as a markdown document.
  ├── Deliver it to the user (or save it wherever they keep working docs).
...
- Not a book summary tool. Use a different approach for general summaries.
- Not a research tool. Use a web-research skill for finding new
