...
Corporate Identity Program design with 50+ deliverables, 20 styles, 20 industries. Generate mockups with the configured Gemini image models in `scripts/cip/generate.py`.

Model source check: as of 2026-06-10, this reference describes the local script defaults only. Before treating Gemini model IDs, names, image limits, or availability as current provider truth, verify the Google Gemini API image-generation/model docs and `scripts/cip/generate.py`.
...
Local script defaults as of 2026-06-10:

- `flash` (default): `gemini-2.5-flash-image` - Fast, cost-effective
- `pro`: `gemini-3-pro-image-preview` - Quality, 4K text rendering

Refresh trigger: if either model is unavailable or renamed, verify current Google Gemini API docs before changing this reference or `scripts/cip/generate.py`.
