#!/usr/bin/env python3
"""Heuristic complexity hotspot scanner for mixed-language repositories."""

from __future__ import annotations

import argparse
import ast
import json
import os
import re
from dataclasses import asdict, dataclass
from pathlib import Path
from typing import Iterable


DEFAULT_EXCLUDES = {
    ".git",
    ".hg",
    ".svn",
    "node_modules",
    "vendor",
    "dist",
    "build",
    ".next",
    ".nuxt",
    "coverage",
    "__pycache__",
    ".venv",
    "venv",
    "target",
    ".turbo",
}

TEXT_EXTENSIONS = {
    ".py",
    ".js",
    ".jsx",
    ".ts",
    ".tsx",
    ".mjs",
    ".cjs",
    ".java",
    ".go",
    ".rb",
    ".php",
    ".cs",
    ".cpp",
    ".cc",
    ".c",
    ".h",
    ".hpp",
    ".swift",
}

LOOP_RE = re.compile(
    r"\b(?:for|while)\b|\b(?:forEach|map|filter|reduce|some|every|find|findIndex)\b\s*\("
)
DO_LOOP_RE = re.compile(r"^do\b")
MEMBERSHIP_RE = re.compile(
    r"(\.includes\s*\(|\.indexOf\s*\(|\.find\s*\(|\.findIndex\s*\(|\bin_array\s*\(|\bcontains\s*\()"
)
SORT_RE = re.compile(r"(\.sort\s*\(|\bsorted\s*\(|\bsort\s*\()")
QUERY_IN_LOOP_RE = re.compile(
    r"\b(fetch\s*\(|axios\s*(?:\.\s*(?:delete|get|head|options|patch|post|put|request)\s*)?\(|request\s*\(|query\s*\(|execute\s*\(|findMany\s*\(|findOne\s*\(|findUnique\s*\(|select\s*\(|where\s*\()",
    re.IGNORECASE,
)
RENDER_HINT_RE = re.compile(
    r"\b(function\s+[A-Z][A-Za-z0-9_]*|const\s+[A-Z][A-Za-z0-9_]*\s*=|export\s+default\s+function\s+[A-Z])"
)
BARE_COMPONENT_BOUNDARY_RE = re.compile(
    r"^(?:async\s+function\b|function\b|const\b|let\b|var\b|class\b|export\b|import\b|return\b|if\b|for\b|while\b|switch\b|try\b|catch\b|do\b)"
)


@dataclass
class Finding:
    path: str
    line: int
    severity: str
    kind: str
    message: str
    suggestion: str


def iter_files(root: Path, excludes: set[str]) -> Iterable[Path]:
    for dirpath, dirnames, filenames in os.walk(root):
        dirnames[:] = [d for d in dirnames if d not in excludes]
        for filename in filenames:
            path = Path(dirpath) / filename
            if path.suffix in TEXT_EXTENSIONS:
                yield path


def read_text(path: Path) -> str | None:
    try:
        return path.read_text(encoding="utf-8")
    except UnicodeDecodeError:
        try:
            return path.read_text(encoding="latin-1")
        except Exception:
            return None
    except Exception:
        return None


def rel(path: Path, root: Path) -> str:
    try:
        return str(path.relative_to(root))
    except ValueError:
        return str(path)


class PythonVisitor(ast.NodeVisitor):
    def __init__(self, path: Path, root: Path) -> None:
        self.path = path
        self.root = root
        self.loop_depth = 0
        self.findings: list[Finding] = []

    def add(self, node: ast.AST, severity: str, kind: str, message: str, suggestion: str) -> None:
        self.findings.append(
            Finding(rel(self.path, self.root), getattr(node, "lineno", 1), severity, kind, message, suggestion)
        )

    def visit_For(self, node: ast.For) -> None:
        self._visit_loop(node)

    def visit_AsyncFor(self, node: ast.AsyncFor) -> None:
        self._visit_loop(node)

    def visit_While(self, node: ast.While) -> None:
        self._visit_loop(node)

    def visit_ListComp(self, node: ast.ListComp) -> None:
        self._visit_comprehension(node, ("elt",))

    def visit_SetComp(self, node: ast.SetComp) -> None:
        self._visit_comprehension(node, ("elt",))

    def visit_DictComp(self, node: ast.DictComp) -> None:
        self._visit_comprehension(node, ("key", "value"))

    def visit_GeneratorExp(self, node: ast.GeneratorExp) -> None:
        self._visit_comprehension(node, ("elt",))

    def _visit_loop(self, node: ast.AST) -> None:
        if self.loop_depth >= 1:
            self.add(
                node,
                "high",
                "nested-loop",
                "Nested loop may create O(n^2) or worse behavior.",
                "Check whether a map/set index, sort+two-pointer pass, grouping, or batching can replace the inner scan.",
            )
        self.loop_depth += 1
        self.generic_visit(node)
        self.loop_depth -= 1

    def _visit_comprehension(self, node: ast.AST, body_fields: tuple[str, ...]) -> None:
        generators = getattr(node, "generators", None)
        if not generators:
            self.generic_visit(node)
            return

        original_depth = self.loop_depth
        try:
            for generator in generators:
                iter_expr = getattr(generator, "iter", None)
                if iter_expr is not None:
                    self.visit(iter_expr)
                if self.loop_depth >= 1:
                    self.add(
                        iter_expr if iter_expr is not None else generator,
                        "high",
                        "nested-loop",
                        "Nested loop may create O(n^2) or worse behavior.",
                        "Check whether a map/set index, sort+two-pointer pass, grouping, or batching can replace the inner scan.",
                    )
                self.loop_depth += 1
                for if_expr in getattr(generator, "ifs", ()):
                    self.visit(if_expr)

            for field in body_fields:
                value = getattr(node, field, None)
                if value is not None:
                    self.visit(value)
        finally:
            self.loop_depth = original_depth

    def visit_Compare(self, node: ast.Compare) -> None:
        if self.loop_depth and any(isinstance(op, (ast.In, ast.NotIn)) for op in node.ops):
            self.add(
                node,
                "medium",
                "membership-in-loop",
                "Membership check inside a loop can become O(n*m) when the right side is a list or computed sequence.",
                "If semantics allow it, build a set or dict once before the loop.",
            )
        self.generic_visit(node)

    def visit_Call(self, node: ast.Call) -> None:
        name = call_name(node.func)
        if self.loop_depth and name in {"sorted", "sort"}:
            self.add(
                node,
                "high",
                "sort-in-loop",
                "Sorting inside a loop is often avoidable repeated O(n log n) work.",
                "Sort once outside the loop, maintain a heap, or use binary search/insertion if intermediate ordering is required.",
            )
        if self.loop_depth and name in {"filter", "map"}:
            self.add(
                node,
                "medium",
                "repeated-scan",
                f"{name}() inside a loop may repeatedly scan a collection.",
                "Consider precomputing an index/grouping or combining passes.",
            )
        if self.loop_depth and name.lower() in {
            "fetch",
            "request",
            "query",
            "execute",
            "find",
            "find_one",
            "find_many",
            "select",
            "where",
        }:
            self.add(
                node,
                "high",
                "io-or-query-in-loop",
                "Potential database/API/file operation inside a loop.",
                "Look for N+1 behavior; batch or preload while preserving auth, filters, ordering, and error handling.",
            )
        self.generic_visit(node)


def call_name(func: ast.AST) -> str:
    if isinstance(func, ast.Name):
        return func.id
    if isinstance(func, ast.Attribute):
        return func.attr
    return ""


def is_do_while_closer(stripped: str, indent: int, loop_stack: list[tuple[int, int, str]], starts_do: bool) -> bool:
    """Heuristically detect the trailing `while` that closes a `do` loop."""
    do_frame = next((frame for frame in reversed(loop_stack) if frame[2] == "do"), None)
    if do_frame is None or indent > do_frame[0]:
        return False

    if starts_do:
        return bool(re.search(r"\bwhile\b", stripped))

    return bool(re.match(r"^(?:\}\s*)?while\b", stripped))


def is_component_boundary(stripped: str) -> bool:
    return bool(BARE_COMPONENT_BOUNDARY_RE.match(stripped)) or stripped.startswith(("}", ")", "]"))


def scan_python(path: Path, root: Path, text: str) -> list[Finding]:
    try:
        tree = ast.parse(text)
    except SyntaxError as exc:
        return [
            Finding(
                rel(path, root),
                exc.lineno or 1,
                "info",
                "parse-error",
                "Python file could not be parsed; falling back to textual scanning only.",
                "Inspect manually if this file is on a hot path.",
            )
        ] + scan_text(path, root, text)
    visitor = PythonVisitor(path, root)
    visitor.visit(tree)
    return visitor.findings


def strip_block_comments(line: str, in_block_comment: bool) -> tuple[str, bool]:
    """Remove C-style block comments while preserving code outside them."""
    cleaned: list[str] = []
    idx = 0

    while idx < len(line):
        if in_block_comment:
            end = line.find("*/", idx)
            if end == -1:
                return "".join(cleaned), True
            idx = end + 2
            in_block_comment = False
            continue

        start = line.find("/*", idx)
        if start == -1:
            cleaned.append(line[idx:])
            break

        cleaned.append(line[idx:start])
        idx = start + 2
        in_block_comment = True

    return "".join(cleaned), in_block_comment


def scan_text(path: Path, root: Path, text: str) -> list[Finding]:
    findings: list[Finding] = []
    lines = text.splitlines()
    loop_stack: list[tuple[int, int, str]] = []
    function_component_ranges = component_ranges(lines) if path.suffix in {".jsx", ".tsx", ".js", ".ts"} else set()
    in_block_comment = False

    for idx, line in enumerate(lines, start=1):
        line, in_block_comment = strip_block_comments(line, in_block_comment)
        stripped = line.strip()
        if not stripped or stripped.startswith(("//", "#", "*", "/*")):
            continue
        # Count tabs and spaces so tab-formatted files keep their loop scope.
        indent = len(line) - len(line.lstrip())
        # Closing delimiters end ordinary loops before we inspect the line.
        # do { ... } while (...) stays active until the trailing while.
        if stripped.startswith(("}", ")", "]")):
            while loop_stack and loop_stack[-1][2] != "do" and loop_stack[-1][0] >= indent:
                loop_stack.pop()
        else:
            loop_stack = [
                (level, lno, kind)
                for level, lno, kind in loop_stack
                if kind == "do" or level < indent
            ]

        starts_do = bool(DO_LOOP_RE.match(stripped))
        closes_do = False
        if starts_do:
            if loop_stack:
                findings.append(
                    Finding(
                        rel(path, root),
                        idx,
                        "high",
                        "nested-or-callback-loop",
                        "Loop or array iteration appears inside another loop/callback.",
                        "Check whether indexing, grouping, batching, or a single-pass algorithm can remove repeated scans.",
                    )
                )
            loop_stack.append((indent, idx, "do"))
            closes_do = bool(re.search(r"\bwhile\b", stripped))
        else:
            closes_do = is_do_while_closer(stripped, indent, loop_stack, starts_do)

        if not starts_do and not closes_do and LOOP_RE.search(stripped):
            if loop_stack:
                findings.append(
                    Finding(
                        rel(path, root),
                        idx,
                        "high",
                        "nested-or-callback-loop",
                        "Loop or array iteration appears inside another loop/callback.",
                        "Check whether indexing, grouping, batching, or a single-pass algorithm can remove repeated scans.",
                    )
                )
            loop_stack.append((indent, idx, "regular"))

        if loop_stack and MEMBERSHIP_RE.search(stripped):
            findings.append(
                Finding(
                    rel(path, root),
                    idx,
                    "medium",
                    "membership-in-loop",
                    "Membership/search operation appears inside iterative code.",
                    "Consider a Set/Map or precomputed lookup if equality and ordering semantics allow it.",
                )
            )

        if loop_stack and SORT_RE.search(stripped):
            findings.append(
                Finding(
                    rel(path, root),
                    idx,
                    "high",
                    "sort-in-loop",
                    "Sort appears inside iterative code.",
                    "Move sorting out of the loop or use a heap/binary-search strategy if intermediate order is needed.",
                )
            )

        if loop_stack and QUERY_IN_LOOP_RE.search(stripped):
            findings.append(
                Finding(
                    rel(path, root),
                    idx,
                    "high",
                    "io-or-query-in-loop",
                    "Potential database/API/file operation inside a loop.",
                    "Look for N+1 behavior; batch or preload while preserving auth, filters, ordering, and error handling.",
                )
            )

        if idx in function_component_ranges and any(token in stripped for token in [".filter(", ".map(", ".sort(", ".reduce("]):
            findings.append(
                Finding(
                    rel(path, root),
                    idx,
                    "medium",
                    "render-derived-work",
                    "Collection transform appears in a likely UI component render path.",
                    "For large collections, consider memoized selectors, server-side derivation, or virtualization.",
                )
            )

        if closes_do:
            while loop_stack and loop_stack[-1][2] != "do":
                loop_stack.pop()
            if loop_stack and loop_stack[-1][2] == "do":
                loop_stack.pop()

    return findings


def component_ranges(lines: list[str]) -> set[int]:
    active_until = 0
    interesting: set[int] = set()
    brace_balance = 0
    expression_balance = 0
    in_component = False
    component_mode = ""
    component_indent = 0

    for idx, line in enumerate(lines, start=1):
        if RENDER_HINT_RE.search(line):
            if "=>" in line:
                arrow_tail = line.split("=>", 1)[1].lstrip()
                if not arrow_tail.startswith("{"):
                    # Wrapped expression-bodied arrows often continue across lines.
                    # Keep scope open only when the arrow tail starts a wrapped expression.
                    if arrow_tail.startswith(("(", "[")):
                        interesting.add(idx)
                        in_component = True
                        component_mode = "expression"
                        active_until = idx + 120
                        expression_balance = delimiter_balance(arrow_tail)
                        if expression_balance <= 0:
                            in_component = False
                            component_mode = ""
                            expression_balance = 0
                        continue
                    if not arrow_tail or arrow_tail.startswith(("//", "/*")):
                        interesting.add(idx)
                        in_component = True
                        component_mode = "bare"
                        active_until = idx + 120
                        component_indent = len(line) - len(line.lstrip())
                        continue
                    interesting.add(idx)
                    in_component = False
                    component_mode = ""
                    continue
            in_component = True
            component_mode = "block"
            active_until = idx + 120
            brace_balance = 0
        if in_component:
            if component_mode == "bare":
                stripped = line.strip()
                # Bare arrow bodies keep the component open until a top-level boundary.
                if stripped and len(line) - len(line.lstrip()) <= component_indent and is_component_boundary(stripped):
                    in_component = False
                    component_mode = ""
                    continue
                interesting.add(idx)
                if idx > active_until:
                    in_component = False
                    component_mode = ""
            elif component_mode == "block":
                interesting.add(idx)
                brace_balance += line.count("{") - line.count("}")
                if idx > active_until or (brace_balance <= 0 and "}" in line):
                    in_component = False
                    component_mode = ""
                    brace_balance = 0
            else:
                interesting.add(idx)
                expression_balance += delimiter_balance(line)
                if idx > active_until or expression_balance <= 0:
                    in_component = False
                    component_mode = ""
                    expression_balance = 0
    return interesting


def delimiter_balance(text: str) -> int:
    return (
        text.count("(")
        + text.count("[")
        + text.count("{")
        - text.count(")")
        - text.count("]")
        - text.count("}")
    )


def dedupe(findings: list[Finding]) -> list[Finding]:
    seen: set[tuple[str, int, str]] = set()
    result: list[Finding] = []
    for finding in findings:
        key = (finding.path, finding.line, finding.kind)
        if key not in seen:
            seen.add(key)
            result.append(finding)
    return result


def severity_rank(finding: Finding) -> tuple[int, str, int]:
    order = {"high": 0, "medium": 1, "info": 2}
    return (order.get(finding.severity, 3), finding.path, finding.line)


def render_markdown(findings: list[Finding]) -> str:
    if not findings:
        return "No obvious complexity hotspots found by heuristic scanning.\n"
    lines = ["# Complexity Hotspots", ""]
    for finding in findings:
        lines.extend(
            [
                f"## {finding.severity.upper()} {finding.kind}",
                f"- Location: `{finding.path}:{finding.line}`",
                f"- Finding: {finding.message}",
                f"- Suggestion: {finding.suggestion}",
                "",
            ]
        )
    return "\n".join(lines)


def main() -> int:
    parser = argparse.ArgumentParser(description="Scan a repository for likely complexity hotspots.")
    parser.add_argument("root", nargs="?", default=".", help="Repository or directory to scan.")
    parser.add_argument("--format", choices=["markdown", "json"], default="markdown")
    parser.add_argument("--exclude", action="append", default=[], help="Additional directory name to exclude.")
    parser.add_argument("--max-findings", type=int, default=80)
    args = parser.parse_args()

    root = Path(args.root).resolve()
    excludes = DEFAULT_EXCLUDES | set(args.exclude)
    findings: list[Finding] = []

    for path in iter_files(root, excludes):
        text = read_text(path)
        if text is None:
            continue
        if path.suffix == ".py":
            findings.extend(scan_python(path, root, text))
        else:
            findings.extend(scan_text(path, root, text))

    findings = sorted(dedupe(findings), key=severity_rank)[: args.max_findings]
    if args.format == "json":
        print(json.dumps([asdict(f) for f in findings], indent=2))
    else:
        print(render_markdown(findings))
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
