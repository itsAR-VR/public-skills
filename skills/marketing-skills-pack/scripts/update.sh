#!/usr/bin/env bash
# marketing-skills-pack/scripts/update.sh
# Refresh the Corey Haines Marketing Skills marketplace pack to the latest version.
#
...
if ! command -v npx >/dev/null 2>&1; then
  echo "[marketing-skills-pack] WARN: npx not available — skipping marketplace refresh."
  exit 1
fi
...
if ! curl -fsS -m 3 -o /dev/null https://registry.npmjs.org/skills 2>/dev/null; then
  echo "[marketing-skills-pack] INFO: npm registry unreachable — skipping refresh."
  exit 0
fi

echo "[marketing-skills-pack] Refreshing $PACKAGE..."
...
if npx -y skills@latest update "$PACKAGE" -g -y 2>/tmp/marketing-skills-update.log; then
  echo "[marketing-skills-pack] Update OK."
else
  echo "[marketing-skills-pack] update failed; falling back to add (with --copy)..."
  if npx -y skills@latest add "$PACKAGE" -g -y --copy; then
    echo "[marketing-skills-pack] Refresh via add OK."
  else
    echo "[marketing-skills-pack] WARN: refresh failed — pack remains at last installed version."
    cat /tmp/marketing-skills-update.log 2>/dev/null || true
