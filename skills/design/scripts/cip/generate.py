...
# -*- coding: utf-8 -*-
"""
CIP Design Generator - Generate corporate identity mockups using Gemini.

Uses the configured Gemini image models for high-quality mockups.
...
# Model options. Local defaults reviewed 2026-06-10; refresh from current Google
# Gemini API docs if either model is unavailable, renamed, or changes capabilities.
MODELS = {
    "flash": "gemini-2.5-flash-image",      # Default local image model
    "pro": "gemini-3-pro-image-preview"      # Pro local image model
}
DEFAULT_MODEL = "flash"
...
def generate_with_nano_banana(prompt_data, output_dir=None, model_key="flash", aspect_ratio="1:1", logo_image=None):
    """Generate image using the configured Gemini image model.
    ...
    try:
        ...
        # Use generate_content with response_modalities=['IMAGE'] for Gemini image generation.
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate CIP mockups using configured Gemini image models",
