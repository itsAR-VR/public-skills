...
Model/source check (2026-06-10): model names, defaults, accepted media roles, params, and Marketing Studio mode support are operational guidance. Before relying on a model-specific fact, refresh it with `higgsfield model list --json` and `higgsfield model get <job_set_type> --json`; update this skill if those commands fail or contradict the guidance below.
