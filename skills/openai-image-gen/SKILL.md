...
description: "FALLBACK ONLY (default: Higgsfield via `/higgsfield:generate`). Batch-generate images via OpenAI Images API. Random prompt sampler + `index.html` gallery."
...
> **Routing rule (2026-05-11):** Default image/video generation in this workspace is **Higgsfield AI** via `/higgsfield:generate` (also `/higgsfield:product-photoshoot` for branded product imagery). Use **this** skill only when the user explicitly names the OpenAI Images API, or when Higgsfield cannot fulfill the request.
