...
| icon-design | SVG icon generation (15 styles, configured Gemini text model) | SKILL.md + 1 reference + 1 script |
...
- Create logos with the configured Gemini image model
...
- Generate mockups with/without logo using the configured Gemini image model
...
- Generate SVG icons with the configured Gemini text model
