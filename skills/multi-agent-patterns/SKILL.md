...
description: Design multi-agent architectures for complex tasks. Use when single-agent context limits are exceeded, tasks decompose into parallel subtasks, or specialized agents improve quality; covers supervisor/orchestrator, swarm, hierarchical, handoff, context isolation, and consensus patterns.
...
Multi-agent systems consume significantly more tokens than single-agent approaches. The original source snapshot used these planning multipliers:

Source check: 2026-06-10. Treat the multipliers and benchmark percentages in this section as planning heuristics from the original 2025-12-20 source snapshot, not evergreen production estimates. Refresh against current official model/provider documentation and fresh eval data before quoting them as current.
...
Critically, upgrading to better models often provides larger performance gains than doubling token budgets. Current model-specific examples are intentionally delegated to dated provider references: use `skills/prompt-generation/references/claude-fable-5.md` for Anthropic model facts and current official OpenAI documentation for OpenAI model facts before naming a model, context window, feature mode, or price. This suggests model selection and multi-agent architecture are complementary strategies.
