...
metadata:
  ...
  source_file: skill-claymation-explainer.json
  hyperagent_name: "Claymation Explainer"
  exported_at: "2026-05-28T17:10:52.503Z"
  auth_type: "none"
  tags:
    - "video"
    - "claymation"
  ...
  when_to_use: >-
    ...
    explainer or ad video built from multiple consistent shots with a voiceover — especially
    "humans + agents collaborating" sizzle pieces, founder/brand explainers, or product ads.
    Also use whenever a multi-scene Veo production needs locked character/scene consistency
    and a single clean VO track.
---
...
Use the bundled `scripts/assemble.py` (preferred) or the raw command below.
...
Helper:
```
python3 skills/claymation-explainer/scripts/assemble.py \
...
7. FFmpeg: concat a=0 + map ONLY the VO; trim/normalize/fade (use scripts/assemble.py).
