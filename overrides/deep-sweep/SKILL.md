...
Load `references/model-currency.md` before hardcoding or changing any model ID,
verifier route, effort control, or Claude Code alias. It contains the dated
official-source snapshot, refresh triggers, and Fable 5 / Opus fallback notes.
...
- Analyzes the problem using provider-appropriate high effort or adaptive reasoning
...
**Target-phase input:** when the invocation names an existing
`docs/planning/<name>/` directory containing a `plan.md` as the target —
numbered `phase-{N}` or a named plan directory like `document-room/`,
both of which a repository may allow (for example, an orchestrator pinning the
phase it planned) — resolve, validate, and announce that directory during
Phase 0 preflight, BEFORE the research wave or any other artifact write,
and use that exact path for every artifact (`research-brief.md`,
`deep-sweep-findings.md`, subphase plans), verifier prompt, and nested
review. Consolidate INTO that directory in place — correct its root
`plan.md` and subphase plans, do not mint a new phase number. Only when no
target is named:
