---
name: memory-systems
description: Design and implement memory architectures for agent systems. Use when building agents that need persistent memory across sessions, entity consistency, vector RAG, knowledge graphs, temporal knowledge graphs, or structured retrieval over accumulated knowledge.
related_skills: [memory-tree, context-optimization, context-compression, multi-agent-patterns, session-handoff]
---
...
The Deep Memory Retrieval (DMR) benchmark snapshot recorded in this skill's original 2025-12-20 draft compared memory architectures as follows. Treat exact percentages, latency numbers, and model-specific baselines as dated evidence; refresh against the current benchmark source and provider documentation before citing them as current truth.
...
In that snapshot, Zep demonstrated a 90% reduction in retrieval latency compared to the same benchmark's full-context baseline (2.58s vs 28.9s). This efficiency comes from retrieving only relevant subgraphs rather than entire context history.

The same snapshot reported approximately 20-35% GraphRAG accuracy gains over baseline RAG in complex reasoning tasks and hallucination reduction up to 30% through community-based summarization.

Refresh trigger: when choosing a production memory stack, citing benchmark numbers, or comparing against current model context windows or latency, rerun the source check and record the check date in the surrounding implementation notes.
