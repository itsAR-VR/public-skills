---
name: video
description: "FALLBACK ONLY (default: Higgsfield via `/higgsfield:generate`). When the user wants to create, generate, or produce video content using AI tools or programmatic frameworks. Also use when the user mentions 'video production,' 'AI video,' 'Remotion,' 'Hyperframes,' 'HeyGen,' 'Synthesia,' 'Veo,' 'Runway,' 'Kling,' 'Pika,' 'video generation,' 'AI avatar,' 'talking head video,' 'programmatic video,' 'video template,' 'explainer video,' 'product demo video,' 'video pipeline,' or 'make me a video.' Use this for video creation, generation, and production workflows. For video content strategy and what to post, see social-content. For paid video ad creative, see ad-creative."
metadata:
  version: 1.0.0
---

> **Routing rule (2026-05-11):** Default image/video generation in this workspace is **Higgsfield AI** via `/higgsfield:generate`. Provider names retained in this skill's description (Remotion, Hyperframes, HeyGen, Synthesia, Veo, Runway, Kling, Pika) stay for keyword routing — but the executor is Higgsfield. Use **this** skill only when the user explicitly insists on a non-Higgsfield workflow.
...
**Volatile tool facts (source check dated 2026-06-11):**
Before quoting model names, availability, output limits, pricing, plan caps, or MCP/API support, check the provider's current official docs or pricing page. Video model specs change quickly; use cached numbers only as examples after dating them.
...
Use this as a routing table, not a cached spec sheet:

| Provider | Best For | Check Before Recommending |
|----------|----------|---------------------------|
| **Veo** (Google) | High-quality cinematic clips, native audio when available | Current Gemini/API docs for model name, availability, duration, resolution, audio, and price |
| **Runway** | Motion control, image-to-video, temporal consistency | Current Runway docs/pricing for model version, duration, resolution, and credit cost |
| **Kling** | Volume production and longer narrative attempts | Current Kling developer docs/pricing for supported duration, quality tier, audio, and per-second cost |
| **Pika** | Fast iteration, effects, social-style clips | Current Pika docs/pricing for API access path, clip limits, supported effects, and credits |

**Sora (OpenAI)**: check current OpenAI video-generation docs and account availability before recommending it.
...
### HeyGen (recommended — verify MCP/API support)

Strong fit for avatar-led explainers, tutorials, localization, and personalized outreach. Verify the current avatar library, language coverage, MCP/API availability, plan limits, and pricing before promising delivery.

**Agent integration:** HeyGen publishes MCP/API docs for agent-driven video generation. Confirm the user's account has the required plan, OAuth/API access, and remaining credits before starting.

**Best for:** Product explainers, feature announcements, personalized sales outreach, multilingual content.

**Custom avatars:** Follow HeyGen's current capture and upload requirements for custom avatar creation. Confirm consent, likeness rights, and plan eligibility before generating.
...
| Tool | Type | MCP | Guide |
|------|------|:---:|-------|
| **HeyGen** | AI avatars | Verify | [HeyGen MCP docs](https://developers.heygen.com/mcp/overview) |
| **Hyperframes** | Programmatic video | - | [HyperFrames docs](https://hyperframes.heygen.com/) |
