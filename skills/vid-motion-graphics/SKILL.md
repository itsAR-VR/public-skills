...
description: Generates motion graphics videos (MP4) from a content brief. Multi-scene HTML/CSS animations rendered frame-by-frame in headless Chromium via Browser Harness, assembled with FFmpeg. 1080×1080 default, 16:9 (1920×1080) and 9:16 (1080×1920) supported. 5 style presets. Trigger when user says "create a video", "motion graphic", "animated video", "make a reel", "create an explainer", "animated infographic", or "short video".
...
Generates multi-scene motion graphics as MP4. Renders HTML/CSS animations in headless Chromium via Browser Harness (Web Animations API frame-seeking), assembles PNG frames with FFmpeg. No React, no AI APIs, no Python — zero new dependencies beyond the graphic-gif family.
...
3. **`window.__videoReady = true` only inside `document.fonts.ready.then(...)`.** Never set synchronously — fonts must load before Browser Harness captures frame 1 or text renders with fallback fonts.
4. **Expose `window.__stopPreview()`.** The browser's rAF preview loop races with Browser Harness's evaluate/screenshot calls. `capture-frames.mjs` calls `__stopPreview()` before the frame loop. Always include it.
...
// ── Main render function — called by Browser Harness once per frame ────────────────
...
// ── Preview loop — stopped by Browser Harness before frame capture ─────────────────
...
The script installs Browser Harness on first run (~200MB Chromium), captures all frames, then assembles MP4 with FFmpeg.
