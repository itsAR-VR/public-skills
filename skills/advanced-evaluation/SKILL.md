...
related_skills: [evaluation, comparative-analysis-orchestrator, recursive-reasoning-operator, qa-test-planner]
---
...
**Evidence-First Justification**: All scoring prompts must require observable evidence and a concise justification before the score. Do not ask for private reasoning; ask for evidence-backed rationale that can be audited.
...
## Output Format
JSON with per-criterion comparison, overall winner, confidence (0-1), and evidence-backed rationale.
```
...
1. **Always require justification before scores** - Use evidence-backed rationale, not private reasoning
