...
**Key behaviors:**
- Uses **Browser Harness MCP** to interact with the live application
...
claude -p --model opus --allowedTools "Read,Bash,mcp__browser-harness__*" "You are an Evaluator. Read EVALUATOR_PROMPT.md. Test the live app at http://localhost:3000. Score against the rubric. Write feedback to feedback-001.md"
...
| `GAN_EVAL_MODE` | `browser-harness` | `browser-harness`, `screenshot`, or `code-only` |

### Evaluation Modes

| Mode | Tools | Best For |
|------|-------|----------|
| `browser-harness` | Browser MCP + live interaction | Full-stack apps with UI |
...
4. **Evaluator testing superficially** — The evaluator must use Browser Harness to **interact** with the live app, not just screenshot it. Click buttons, fill forms, test error states.
