...
Generates multi-scene motion graphics as MP4 from a content brief. HTML/CSS animations rendered frame-by-frame in headless Chromium via Browser Harness, assembled with FFmpeg. No React, no AI APIs, no Python.
...
3. Captures frames via Browser Harness + Web Animations API seeking
...
- Node.js (for Browser Harness frame capture)
...
All top SkillsMP motion-graphic skills require Remotion (React build step) or AI video APIs (Runway, Kling — cost + rate limits). This skill uses the proven HTML/CSS → Browser Harness → FFmpeg pipeline from the `graphic-gif` family: zero new dependencies, pixel-perfect output, full CSS control.
