...
- Published LLM-as-judge studies report position bias across model families
- Bias rates vary by model release and prompt, so measure on the current judge
...
def get_evaluator_model(generator_model):
    ...
    # As of 2026-06-10, fill these from current provider docs or local model policy.
    if 'gpt' in generator_model.lower():
        return CROSS_FAMILY_EVALUATOR_MODEL
