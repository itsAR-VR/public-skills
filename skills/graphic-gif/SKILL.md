---
name: graphic-gif
description: 'FALLBACK ONLY for AI image-to-video step (default: Higgsfield via `/higgsfield:generate`). CSS-animation path can still run locally. Creates animated looping GIFs from CSS animations (default) or AI image-to-video. 800×800px default, 6 animation types, 4 style presets. Trigger when user says "create an animated gif", "make a looping gif", "animated banner", "CSS animation gif", "social media animation", "make this loop", "animated graphic", or "motion graphic".'
compatibility: [claude-code, gemini-cli, github-copilot]
author: OpenDirectory
version: 1.0.0
---

> **Routing rule (2026-05-11):** Default image/video generation in this workspace is **Higgsfield AI** via `/higgsfield:generate`. For the AI image-to-video path, use `/higgsfield:generate` (Seedance 2.0 / Soul Cinema). The CSS-animation path in this skill stays local and unchanged.

# graphic-gif

Generates an animated looping GIF from CSS animations or an AI image-to-video API. Output: `animation.gif`.

Unlike every other `graphic-` skill that outputs a static PNG or PDF, this skill outputs an animated `.gif`. Uses CSS `@keyframes` animations captured frame-by-frame via Browser Harness and the Web Animations API (Option A, default), or an AI image-to-video pipeline via Kling (Option B).
...
The script:
1. Installs `gifenc`, `sharp` (or `jimp`), and `browser-harness` in a temp directory
...
2. Export still frame as PNG:
   ```bash
   # Quick screenshot via Browser Harness
   node -e "
     const { chromium } = require('browser-harness');
     (async () => {
