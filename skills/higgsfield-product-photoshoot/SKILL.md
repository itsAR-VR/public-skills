...
description: |
  ...
  prompt enhancement on GPT Image 2 / gpt_image_2 (source-checked 2026-06-10).
  Entry point for professional brand/product visuals.
...
Model/backend facts in this skill were source-checked on 2026-06-10 against Higgsfield's GPT Image 2 and CLI pages (`https://higgsfield.ai/gpt-2`, `https://higgsfield.ai/cli`). Refresh this skill if Higgsfield changes product-photoshoot routing, CLI flags, resolution/aspect-ratio support, or if official OpenAI image-generation docs contradict the `gpt_image_2` naming.
...
As of the source check above, use `2k` for every product-photoshoot job.
...
- Does not auto-pick a different image-gen model. While the source check above holds, always use `gpt_image_2`.
