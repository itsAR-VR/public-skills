...
All scene timing is driven by `window.renderFrame(t)` — a pure JS function called by Browser Harness once per frame. It computes `opacity`/`transform` directly from the millisecond timestamp. No CSS `@keyframes`, no `animation-delay`, no Web Animations API seeking.
...
// Preview rAF loop — stopped by Browser Harness before frame capture
