# Skill Judge — Scoring Detail

Load this file before scoring any skill. It carries the full per-dimension
scoring tables, the common failure patterns, and the report template. The
philosophy, protocol, and quick checklist live in `SKILL.md`.
...
## D9: Refresh Boundary & Evalability (10 points)

Does the Skill keep volatile facts honest and its quality checkable?

Skills outlive the model facts inside them. A skill that hardcodes a model
ID, price, context window, or CLI flag without a dated source check and a
named refresh trigger ships a future bug. A skill with no validation hook
cannot be regression-tested when its domain shifts.

| Score | Criteria |
|-------|----------|
| 0-3 | Undated model/price/flag claims; no validation hooks of any kind |
| 4-6 | Some claims dated or some validation present, but inconsistent |
| 7-8 | Volatile claims dated with named refresh triggers, OR validation commands / `evals/evals.json` present |
| 9-10 | Both: dated claims with named refresh triggers AND validation hooks; structural vs behavioral proof explicitly labeled |

**Check for**:
- Every model ID, price, context-window, version, or feature-flag claim
  carries a source-check date and names what event forces a refresh
  ("stale after any Anthropic model release", "verify against the current
  Claude Code changelog").
- A validation surface exists: runnable commands, `scripts/` checks, or an
  `evals/evals.json` with assertable cases.
- Proof claims are labeled structural (file exists, frontmatter parses,
  command exits 0) vs behavioral (an agent following the skill produces the
  intended output). Passing structural checks off as behavioral proof is a
  deduction.
- Volatile facts are pointed at a dated per-model reference (e.g.,
  `prompt-generation` `references/claude-fable-5.md`) instead of being
  inlined into long-lived skill text.
...
| D9: Refresh Boundary & Evalability | X | 10 | |
