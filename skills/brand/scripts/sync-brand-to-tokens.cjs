...
function resolveGenerateTokensScript() {
  const candidates = [
    path.resolve(__dirname, '..', '..', 'design-system', 'scripts', 'generate-tokens.cjs'),
    path.join(os.homedir(), '.codex', 'skills', GENERATE_TOKENS_SCRIPT),
    path.join(os.homedir(), '.claude', 'skills', GENERATE_TOKENS_SCRIPT),
    path.resolve(process.cwd(), 'skills', GENERATE_TOKENS_SCRIPT),
  ];
...
function nameFromLabel(label) {
  return label
    .replace(/\*\*/g, '')
    .replace(/\b(primary|secondary|accent|color|dark|light)\b/gi, '')
    .trim()
    .toLowerCase()
    .replace(/\s+/g, '-') || 'brand';
}

function assignColor(target, rawName, hex) {
  const name = rawName.trim().toLowerCase();
  if (name.includes('dark')) target.dark = hex;
  else if (name.includes('light')) target.light = hex;
  else {
    target.name = nameFromLabel(rawName);
    target.base = hex;
  }
}
...
function extractColorsFromMarkdown(content) {
  ...
  if (quickRefMatch) {
    colors.primary.name = quickRefMatch[2] ? quickRefMatch[2].toLowerCase().replace(/\s+/g, '-') : 'primary';
    colors.primary.base = `#${quickRefMatch[1]}`;
  }

  const secondaryMatch = content.match(/Secondary Color\s*\|\s*#([A-Fa-f0-9]{6})(?:\s*\(([^)]+)\))?/);
  if (secondaryMatch) {
    colors.secondary.name = secondaryMatch[2] ? secondaryMatch[2].toLowerCase().replace(/\s+/g, '-') : 'secondary';
    colors.secondary.base = `#${secondaryMatch[1]}`;
  }

  const accentMatch = content.match(/Accent Color\s*\|\s*#([A-Fa-f0-9]{6})(?:\s*\(([^)]+)\))?/);
  if (accentMatch) {
    colors.accent.name = accentMatch[2] ? accentMatch[2].toLowerCase().replace(/\s+/g, '-') : 'accent';
  ...
  if (primarySection) {
    const hexMatches = primarySection[0].matchAll(/\|\s*(?:\*\*)?([^*|\n]+?)(?:\*\*)?\s*\|\s*#([A-Fa-f0-9]{6})/g);
    for (const match of hexMatches) {
      const hex = `#${match[2]}`;
      assignColor(colors.primary, match[1], hex);
    }
  }
  ...
  if (secondarySection) {
    const hexMatches = secondarySection[0].matchAll(/\|\s*(?:\*\*)?([^*|\n]+?)(?:\*\*)?\s*\|\s*#([A-Fa-f0-9]{6})/g);
    for (const match of hexMatches) {
      const name = match[1].trim().toLowerCase();
      const hex = `#${match[2]}`;
      if (name.includes('accent') && !colors.accent.base) {
        assignColor(colors.accent, match[1], hex);
      } else {
        assignColor(colors.secondary, match[1], hex);
      }
    }
  }
  ...
  if (accentSection) {
    const hexMatches = accentSection[0].matchAll(/\|\s*(?:\*\*)?([^*|\n]+?)(?:\*\*)?\s*\|\s*#([A-Fa-f0-9]{6})/g);
    for (const match of hexMatches) {
      const hex = `#${match[2]}`;
      assignColor(colors.accent, match[1], hex);
    }
...
function main() {
  ...
  // Regenerate CSS
  const generateScript = resolveGenerateTokensScript();
  if (generateScript) {
    try {
