...
- Keep `SKILL.md` lean; the executing model is already capable.
- Skills run on both Codex and Claude; write "the executing model is already
  capable", not provider-specific phrasing.
- Never instruct the model to show, echo, or transcribe its internal
  reasoning — on Claude Fable 5 this triggers a refusal; rely on output
  restatement of goals and evidence instead.
- Date any model ID, price, context-window, or feature claim and name its
  refresh trigger; prefer pointing at `prompt-generation`
  `references/claude-fable-5.md` over inlining Anthropic model facts.
- Agentic or multi-session skills must state boundaries (what NOT to do) and
  a memory path with one-lesson-per-file, update-don't-duplicate rules.
