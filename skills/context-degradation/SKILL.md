...
description: >
  Diagnose and mitigate context degradation in agent systems: lost-in-middle,
  context poisoning, distraction, confusion, and clash. Use when long-context
  agents degrade, hallucinations persist, retrieved context conflicts, or
  production context engineering needs reliability checks.
related_skills: [context-compression, context-fundamentals, context-optimization, memory-systems, session-handoff]
---
...
The RULER benchmark delivers sobering findings: model context-window claims do not guarantee reliable long-context task performance. Near-perfect scores on simple needle-in-haystack tests do not translate to real long-context understanding.

**Source Check Required**
Last refreshed: 2026-06-10.

Model-specific degradation thresholds, context windows, prices, modes, and benchmark rankings drift quickly. Before making model-specific claims, check the current official model documentation and current benchmark sources:

- OpenAI model documentation for available model IDs, context limits, modes, and pricing.
- Anthropic model and context-window documentation for Claude context limits and feature flags.
- Google Gemini model documentation for Gemini context limits, modalities, and deprecation status.
- The current benchmark paper or leaderboard before quoting RULER, SWE-bench, MMMU-Pro, or similar scores.

Use this section's model-agnostic method unless those dated sources have been checked in the current task. Treat provider marketing context windows as capacity limits, not proof of robust recall at every position.
