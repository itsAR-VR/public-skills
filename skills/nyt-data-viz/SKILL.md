...
description: >-
  ...
  any chart render at Times grade. Use whenever generating charts, dashboards, reports, or
  data-driven webpages.
metadata:
  source_repo: https://github.com/alexmcdonnell-airtable/hyperagent-public-skills
  source_commit: a1349c1
  source_file: skill-nyt-data-viz.json
  hyperagent_name: "nyt-data-viz"
  exported_at: "2026-06-01T13:39:09.428Z"
  auth_type: "none"
  tags:
    - "data-viz"
    - "design"
    ...
    - "editorial"
  when_to_use: >-
    Use whenever the agent generates a chart, dashboard, data-viz webpage, slide with data,
    or any artifact that contains charts. The skill encodes house-style rules that lift
    default Plotly/Chart.js output to NYT-grade.
---
## NYT-discipline Data Visualization

Produce New York Times / Upshot–grade charts and dashboards: the typography, color restraint, chart-type judgment, and annotation discipline of the NYT graphics desk — for **both** static charts (Observable Plot via `scripts/render.py`) and hand-built **interactive** D3 dashboards (via `scripts/interactive.py`).
...
1. **Color.** One hero accent for the series that matters; everything else grey. Monotonic-luminance sequential ramps only — never rainbow/jet/HSV-equidistant. Diverging only with a meaningful zero. Categorical caps at ~7 hues; beyond that, small-multiple by category. (`scripts/palette.py`)
2. **Typography.** Playfair Display (display serif) + Libre Franklin (sans labels/annotation) + Source Serif 4 (body). `tabular-nums` on ALL numeric labels so digits don't jitter. (`scripts/typography.py`)
3. **Chart choice.** Line first for time; bar second; never pie; never dual y-axes; bars start at zero. Past ~5 series → small multiples. (`scripts/chart_selector.py`)
4. **Annotation.** Declarative headline (a sentence, not a label) + subtitle naming unit & timeframe + source line. Label series DIRECTLY at endpoints (no legends). Annotate inflections in place. Humanized axis labels. (`scripts/annotate.py`)
5. **Archie Tse rule.** Crucial info visible without interaction; mobile-test at 375px first.

### Scripts
- `scripts/palette.py`, `scripts/typography.py`, `scripts/chart_selector.py`, `scripts/annotate.py` — discipline helpers/linters.
- `scripts/render.py` — renders a STATIC chart as a standalone Observable Plot HTML page.
- `scripts/interactive.py` — **NYT chrome + interaction primitives** for hand-built D3 dashboards: `INTERACTIVE_CSS`, `HELPER_JS` (gliding tooltip + Voronoi hover), `scaffold(title, body_html, extra_js)`, and `PREPUBLISH_CHECKLIST`. Import it and build per-build render functions on top of `scaffold()`.
...
- **Bump / ranking** — one accent for the leader, grey for the chase (scripts/palette.py "hero" rule applied to motion).
...
### Reusable hover helper (from `scripts/interactive.py`)
```js
