...
# -*- coding: utf-8 -*-
"""
Logo Generation Script using the configured Gemini image models.
Model source check: local defaults reviewed 2026-06-10; verify current
Google Gemini API image-generation/model docs before treating model IDs,
names, limits, or availability as current provider truth.

Models as local defaults reviewed 2026-06-10:
...
Usage:
    ...
    python generate.py --brand "TechFlow" --pro  # Use the configured pro model
...
# Gemini image model configurations for image generation.
# Local defaults reviewed 2026-06-10; refresh from current Google Gemini API docs
# if either model is unavailable, renamed, or changes capabilities.
GEMINI_FLASH = "gemini-2.5-flash-image"  # Default local image model
GEMINI_PRO = "gemini-3-pro-image-preview"  # Pro local image model
...
def generate_logo(prompt, style=None, industry=None, brand_name=None,
    ...
    model_label = "pro local default (gemini-3-pro-image-preview)" if use_pro else "default local model (gemini-2.5-flash-image)"
...
def main():
    parser = argparse.ArgumentParser(description="Generate logos using configured Gemini image models")
    ...
    parser.add_argument("--pro", action="store_true", help="Use the configured pro local default (gemini-3-pro-image-preview)")
