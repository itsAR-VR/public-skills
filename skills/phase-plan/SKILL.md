...
related_skills: [skill-oracle, phase-gaps, phase-review, verify, phase-implement, terminus-maximus, deep-sweep, deep-build, think, goal-post, ultra-review, deep-clean, browser-harness, gepetto, requirements-clarity]
---
...
When composing phase workflows with other delivery skills, also read
`references/09_PHASE_PIPELINE_PLAYBOOK.md`. It is the canonical short-guide for
combining `skill-oracle`, `think`, `deep-sweep`, `deep-build`, `goal-post`,
review, cleanup, and browser proof without duplicating that logic here.
For beginners, use the playbook to understand the order of operations; use this
file for the exact `phase-plan` artifact contract and validation gates.

This skill is a planner for multi-agent execution. Default output is a
dependency graph of subphases. Each subphase is one of: a vertical feature
slice (DB+API+UI+tests for one user-visible capability) or a horizontal layer
(cross-cutting refactor/migration). Plans default to `vertical` for
`phase_intent: feature`; horizontal otherwise. Backwards-compatible: legacy
plans without slicing front-matter are treated as horizontal.
...
When `default_slice_type: vertical`, this skill composes with the broader
ecosystem at named gates. **Tiered**: required core fires deterministically;
recommended optional fires when conditions match. Source-of-truth catalogs
live in repo-local `skills/ecc-*` directories, `~/.claude/agents/gsd-*`, and
repo-local `skills/superpowers-*/` directories.

### Required core (MUST invoke)

- `/skill-oracle "<feature_id>"` @ Step 5 per slice — populates each
  subphase's Skills Available section.
- `gsd-pattern-mapper` @ Step 5 per slice — maps `surfaces[]` to existing
  codebase analogs.
- `gsd-research-synthesizer` @ Step 4 close — already wired in the GSD
  planning layer.
- `gsd-plan-checker` + `gsd-integration-checker` @ Step 6 — already wired.

### Recommended optional (SHOULD invoke when conditions match)

- ECC `architect` agent @ Step 5 — when a slice has `integration_contracts`;
  cross-feature contract design lens.
- ECC `planner` agent @ Step 5 — when phase has 4+ slices; cross-arch sanity.
- `superpowers-brainstorming` @ Step 4.5 — when slicing decision is
  non-obvious (multiple valid decompositions).
- `superpowers-writing-plans` @ Step 5 — discipline applied to each subphase
  plan.md.
- ECC context `research.md` — persona for the planner (vs `dev.md` for
  execution).
- `gsd-framework-selector` @ Step 4 — when feature introduces new tech (AI
  integration, new framework).
- `gsd-domain-researcher` @ Step 4 — when feature touches business domain
  logic.

### Per-slice tools (fire once per feature_id)

- `/skill-oracle "<feature_id>"` per slice — bespoke skill discovery.
- `mcp__gitnexus__impact` per surface symbol — blast radius per slice.

### Source of truth (avoid rot)

- ECC catalog: repo-local `skills/ecc-*` directories named in `08_PARALLEL_AGENT_ARCHITECTURE.md`
...
If `default_slice_type: vertical` → recommend `/deep-build` with per-slice
gear loop and worktree-per-feature isolation. Each vertical subphase ships
full-stack (DB+API+UI+tests) end-to-end before the next slice starts
internally; phase-atomic ship gate emits one PR with per-slice commits.
For every `feature_id` in `slice_index`, run **per-slice** `/skill-oracle
"<feature_id>"` and **per-slice** `gsd-pattern-mapper` once during Step 5
decomposition — outputs land in that subphase's Skills Available and Approach
