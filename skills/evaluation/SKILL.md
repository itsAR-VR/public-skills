---
name: evaluation
description: "Build evaluation frameworks for agent systems: rubrics, LLM-as-judge, test sets, regression gates, and production monitoring. Use when testing agent performance, validating context engineering choices, comparing agent configurations, or measuring improvements over time."
related_skills: [advanced-evaluation, ab-test-setup, qa-test-planner, comparative-analysis-orchestrator]
---
...
- **Model upgrades can beat token increases**: When current benchmark evidence shows a stronger model improves task success, compare that upgrade against simply raising token budgets on the previous model
...
**Source check (2026-06-10)**: Treat BrowseComp variance numbers and any model-specific comparisons as volatile. Before citing current model names, prices, context windows, feature flags, or "best model" claims, refresh against the benchmark source plus current Anthropic/OpenAI documentation, or delegate model guidance to `skills/prompt-generation/references/claude-fable-5.md` where applicable.
