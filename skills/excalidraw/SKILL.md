...
Excalidraw files are JSON with high token cost but low information density. Single files range from 4k-22k tokens (largest can exceed read tool limits). Reading multiple diagrams quickly exhausts context budget (7 files = 67k tokens loaded into the parent context).
...
Baseline measurements from the original skill, refresh-checked on 2026-06-10. Refresh these figures when target model context windows or tool read limits change:

| Scenario | Without Delegation | With Delegation | Savings |
|----------|-------------------|-----------------|---------|
| Single large file | 22k tokens in parent context | ~500 tokens (subagent summary) | 98% |
| Two-file comparison | 18k tokens in parent context | ~800 tokens (diff summary) | 96% |
| Modification task | 14k tokens in parent context | ~300 tokens (confirmation) | 98% |

**Context pollution impact:**
- Reading all 7 project diagrams: 67k tokens in parent context
