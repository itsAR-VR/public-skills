...
Source check (2026-06-10): this catalog is operational guidance, not a permanent model registry. Before depending on a model name, role, duration, aspect ratio, or parameter, refresh with `higgsfield model list --json` and `higgsfield model get <job_set_type> --json`; update this file when CLI output fails or disagrees with the catalog.
