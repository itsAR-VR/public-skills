...
license: Apache-2.0
related_skills: [ecc-frontend-design, frontend-coding-agent, design-system-starter, tool-design, impeccable]
routing:
  domain_keywords:
    - canvas
    - pdf
    ...
    - card
  intent_patterns:
    - "(?:create|design|generate)\\s+(?:a\\s+)?(?:canvas|pdf|poster|infographic|certificate|badge|card|visual)"
    - "(?:png|pdf|image)\\s+(?:output|export|render|generate)"
  lane: codex-worker
